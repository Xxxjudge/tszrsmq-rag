...
## Cookie 管理（Step 1 前置条件）

推文数据通过 X 已登录 cookie 直接爬取，**比 WebSearch 获得原始推文全文、互动数据和时间戳**。

**Cookie 文件路径：**
```
~/.claude/skills/serenity-ashare/.cookie
```

**获取/更新 cookie 的步骤（cookie 失效时提示用户执行）：**
1. 打开 Chrome，登录 x.com
2. DevTools（`Cmd+Option+I`）→ **Network** 标签 → 刷新页面
3. 点击任意 `api.x.com` 请求 → **Request Headers** → 复制 `cookie:` 整行的值
4. 将复制的内容写入 `~/.claude/skills/serenity-ashare/.cookie`（覆盖旧内容）

**Cookie 失效判断：** Python 脚本返回 HTTP 401 即为失效，立刻停止并提示用户更新。

---

## Step 1a：拉取过去 24 小时推文

使用 **python3.12** 执行以下脚本（不告知用户正在执行）：
...
# 读取 cookie
COOKIE_FILE = os.path.expanduser('~/.claude/skills/serenity-ashare/.cookie')
if not os.path.exists(COOKIE_FILE):
    raise FileNotFoundError('Cookie 文件不存在，请按 Cookie 管理步骤更新')
COOKIE = open(COOKIE_FILE).read().strip()
ct0 = re.search(r'ct0=([a-f0-9]+)', COOKIE)
if not ct0:
    raise ValueError('Cookie 中未找到 ct0，请更新 cookie')
CT0 = ct0.group(1)

# 动态获取最新 bearer token 和 query ID（每次从 main.js 提取，避免硬编码失效）
page = requests.get('https://x.com/', headers={'user-agent': 'Mozilla/5.0'}, timeout=10)
js_url = re.findall(r'https://abs\.twimg\.com/responsive-web/client-web/main\.[^"]+\.js', page.text)
if not js_url:
    raise RuntimeError('无法获取 main.js URL')
js = requests.get(js_url[0], headers={'user-agent': 'Mozilla/5.0'}, timeout=20).text
bearer_match = re.search(r'Bearer (AAAAAAAAAAAAAAA[A-Za-z0-9%+/=]{30,})', js)
BEARER = bearer_match.group(1) if bearer_match else 'AAAAAAAAAAAAAAAAAAAAANRILgAAAAAAnNwIzUejRCOuH5E6I8xnZz4puTs%3D1Zv7ttfk8LF81IUq16cHjhLTvJu4FA33AGWWjCpTnA'
qid_match = re.search(r'queryId:"([a-zA-Z0-9_-]{20,})"[^}]*operationName:"UserTweets"', js)
QID = qid_match.group(1) if qid_match else 'PNd0vlufvrcIwrAnBYKE9g'
...
HEADERS = {
    ...
    'referer': 'https://x.com/aleabitoreddit',
}
USER_ID = '1940360837547565056'  # @aleabitoreddit（稳定不变）

def fetch_tweets(count=40, cursor=None):
    v = {"userId": USER_ID, "count": count, "includePromotedContent": True, "withVoice": True}
    if cursor:
        v["cursor"] = cursor
    feat = {"rweb_tipjar_consumption_enabled": True, "responsive_web_graphql_exclude_directive_enabled": True,
            "verified_phone_label_enabled": False, "creator_subscriptions_tweet_preview_api_enabled": True,
            "responsive_web_graphql_timeline_navigation_enabled": True,
            "responsive_web_graphql_skip_user_profile_image_extensions_enabled": False,
            "communities_web_enable_tweet_community_results_fetch": True, "articles_preview_enabled": True,
            "responsive_web_edit_tweet_api_enabled": True, "view_counts_everywhere_api_enabled": True,
            "longform_notetweets_consumption_enabled": True, "freedom_of_speech_not_reach_fetch_enabled": True,
            "longform_notetweets_rich_text_read_enabled": True, "longform_notetweets_inline_media_enabled": True,
            "responsive_web_enhance_cards_enabled": False}
    r = requests.get(f'https://twitter.com/i/api/graphql/{QID}/UserTweets',
                     headers=HEADERS, params={'variables': json.dumps(v), 'features': json.dumps(feat)}, timeout=15)
    if r.status_code == 401:
        raise PermissionError('COOKIE_EXPIRED')  # 外层捕获并提示用户
    return r.json()

def parse_tweets(data):
    tweets, cursor = [], None
    user_r = data['data']['user']['result']
    tl_key = 'timeline_v2' if 'timeline_v2' in user_r else 'timeline'
    for inst in user_r[tl_key]['timeline']['instructions']:
        ...
        for entry in inst.get('entries', []):
            ...
            if tr.get('__typename') == 'TweetWithVisibilityResults':
                tr = tr.get('tweet', tr)
            lg = tr.get('legacy', {})
            if not lg or not lg.get('created_at') or lg.get('full_text', '').startswith('RT @'):
                continue
            tweets.append({
                'id': lg.get('id_str'),
                'dt': datetime.strptime(lg['created_at'], '%a %b %d %H:%M:%S +0000 %Y').replace(tzinfo=timezone.utc),
...
# 拉取并分类（24h）
raw = fetch_tweets(40)
tweets, cursor = parse_tweets(raw)
if cursor and len(tweets) < 50:
    tweets2, _ = parse_tweets(fetch_tweets(40, cursor))
    tweets.extend(tweets2)

now = datetime.now(timezone.utc)
tweets_24h = [t for t in tweets if t['dt'] > now - timedelta(hours=24)]
tweets_2w  = [t for t in tweets if now - timedelta(days=14) < t['dt'] <= now - timedelta(hours=24)]

# 输出供后续步骤使用
for t in tweets_24h:
    print(f"[24H] {t['dt'].strftime('%Y-%m-%d %H:%M')} UTC ❤{t['likes']} 🔁{t['retweets']} 👀{t['views']}")
    print(t['text'])
    print('---')
```
...
**24h 结果标记：** 将提取内容标记为 `[今日新信号]`，若无内容则标记为 `[今日无新信号]`。

---

## Step 1b：拉取过去 2 周推文

**复用 Step 1a 已获取的 `tweets_2w` 列表**（同一次 API 调用已包含 2 周数据，无需重复请求）。

若 `tweets_2w` 为空（账号近期发帖少），执行补充拉取：

```python
# 仅在 tweets_2w 为空时执行：继续翻页获取更多历史
if not tweets_2w and cursor:
    for _ in range(3):  # 最多再翻 3 页
        more, cursor = parse_tweets(fetch_tweets(40, cursor))
        tweets_2w.extend([t for t in more if now - timedelta(days=14) < t['dt'] <= now - timedelta(hours=24)])
        if not cursor or (more and more[-1]['dt'] < now - timedelta(days=14)):
            break

for t in tweets_2w:
    print(f"[2W] {t['dt'].strftime('%Y-%m-%d %H:%M')} UTC ❤{t['likes']} 🔁{t['retweets']} 👀{t['views']}")
    print(t['text'])
    print('---')
```
...
| 场景 | 处理方式 |
|------|---------|
| Cookie 文件不存在 | 停止运行，提示用户按"Cookie 管理"步骤创建 `~/.claude/skills/serenity-ashare/.cookie` |
| Cookie 失效（401）| 停止运行，提示用户："Cookie 已过期，请重新从浏览器复制并更新 .cookie 文件" |
| 24h 内无新推文 | 第一部分输出"今日暂无新推文"，不影响第二部分 |
| 2周推文为空 | 使用知识库已确认主线兜底，注明"数据来源：知识库，建议访问 x.com/@aleabitoreddit 确认" |
